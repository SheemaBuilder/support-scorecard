...
This project includes a complete Zendesk API integration:

- **Backend Server**: `npm run server` - Starts the backend proxy server on port 3001
- **Full Development**: `npm run dev:full` - Runs both frontend and backend together
- **No Dummy Data**: If backend is not available, the app shows empty states (no dummy/demo data)

### Backend Setup

1. Ensure `.env` file has Zendesk credentials
2. Install backend dependencies: `cd server && npm install`
3. Start backend: `npm run server`
4. Test API: Visit `/test-api.html` in your browser
